...
• Build 018 introduces GPT fallback across all flows (guided + free-text), removes rigid input fields, adds flexible time parsing, and unifies automatic schema repair for ambiguous user input. Next required sub-step: 3C.3 — Unified Media→GPT→Schema pipeline.
[2025-11-22] — Build 018 — GPT Fallback Engine + Flexible Input Framework

Summary:
Implemented a universal GPT-based fallback layer to support flexible natural-language input across all guided flows (sleep, exercise, food). This build replaces rigid numeric-only prompts with GPT normalization capable of interpreting messy or ambiguous user text. All flows remain local-first; GPT only triggers when parsing fails.

Status:
⚠️ Awaiting Testing
Build 018 code is fully integrated but NOT validated yet.
No QA, no regression, no flow confirmation done.
